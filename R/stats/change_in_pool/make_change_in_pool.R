...
make_change_in_pool <- function(mypool,var.col){
    ...
    for (i in 1:length(prod$Date)) {
        ...
        prod$prev_biom[i] <- mypool[mypool$Ring == prod$Ring[i] &
                                                     as.numeric(mypool$Date-prod$Start_date[i])==0, var.col]
    }
    ...
    prod$production_flux <- (prod[,var.col] - prod$prev_biom) / prod$length 
