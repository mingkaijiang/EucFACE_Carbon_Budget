generate_stats_ratio <- function(stat.model) {
    ...
    s.sdc <- treatment_effect_ratio_statistics(inDF=standing_dead_c_pool, 
                                                var.cond="pool", var.col=3,
                                                date.as.factor=T)
    
    ### Leaf litter C pool
    s.litc <- treatment_effect_ratio_statistics(inDF=leaflitter_pool, 
    ...
    ### Overstorey GPP
    s.o.gpp <- treatment_effect_ratio_statistics(inDF=overstorey_gpp_flux, 
                                               var.cond="ann.flux", var.col=3,
                                               date.as.factor=T)
    
    ### Understorey GPP
    s.u.gpp <- treatment_effect_ratio_statistics(inDF=understorey_gpp_flux, 
                                               var.cond="ann.flux", var.col=3,
                                               date.as.factor=T)
    ### Overstorey Leaf respiration
    s.rleaf <- treatment_effect_ratio_statistics(inDF=overstorey_leaf_respiration_flux, 
                                               var.cond="ann.flux", var.col=3,
                                               date.as.factor=T)
    
    ### Wood respiration
    s.rwood <- treatment_effect_ratio_statistics(inDF=wood_respiration_flux, 
