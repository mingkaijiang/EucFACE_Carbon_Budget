generate_stats_raw_data <- function() {
    ...
    ### LAI
    s.lai <- make_lai_variable(return.decision="stats",
                               trt.effect="abs",
                               stat.model="interaction")
    
    ### SLA
    s.sla <- make_sla_variable(return.decision="stats",
                               trt.effect="abs",
                               stat.model="interaction")
    
    ### Soil bulk density at 3 depths
    ### unit is kg m-3
    s.soil.bk <- make_soil_bulk_density(return.decision="stats",
                                        trt.effect="abs",
                                        stat.model="interaction")
    ...
    s.rsoil <- make_soil_respiration_flux(return.decision="stats",
                                          trt.effect="abs",
                                          stat.model="interaction")
    ...
    s.fras <- make_frass_production_flux(return.decision="stats",
                                         trt.effect="abs",
                                         stat.model="interaction")
    ...
    ### understorey SLA
    s.ua.sla <- make_understorey_sla_variable(return.decision="stats",
                                              trt.effect="abs",
                                              stat.model="interaction")
    ...
    s.uac2 <- make_understorey_aboveground_c_pool_2(c_fraction,
                                                    return.decision="stats",
                                                    trt.effect="abs",
                                                    stat.model="interaction")
    ...
    s.ch4 <- make_methane_flux(return.decision="stats",
                               trt.effect="abs",
                               stat.model="interaction")
