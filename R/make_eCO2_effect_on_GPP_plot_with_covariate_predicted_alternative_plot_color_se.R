make_eCO2_effect_on_GPP_plot_with_covariate_predicted_alternative_plot_color <- function() {
    ...
    ### !!! first possibility to reduce confidence interval
    myDF$sd <- with(myDF, se1 * sqrt((Date_Df + 1)))
    
    ### !!! second possibility to reduce confidence interval
    myDF$sd <- myDF$se1
    ...
    ### Plotting
    pdf("Output/eco2_effect_on_gpp_and_subsequent_fluxes_pools_with_covariate_bootstrapped_se.pdf", width=8, height=6)
    #plot(p1)
    plot(p3)
    #plot(p2)
    dev.off()
