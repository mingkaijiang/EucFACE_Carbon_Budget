nep_gap_bootstrap_plot <- function(inDF) {
    ...
    inoutDF <- as.data.frame(inDF$inout[,c("term", "aCO2", "eCO2", "aCO2_conf", "eCO2_conf")])
    nppDF <- as.data.frame(inDF$npp[,c("term", "aCO2", "eCO2", "aCO2_conf", "eCO2_conf")])
    deltaDF <- as.data.frame(inDF$delta_pool[,c("term", "aCO2", "eCO2", "aCO2_conf", "eCO2_conf")])
    inoutDF$aCO2_sd <- inoutDF$aCO2_conf / 1.96
    inoutDF$eCO2_sd <- inoutDF$eCO2_conf / 1.96
    nppDF$aCO2_sd <- nppDF$aCO2_conf / 1.96
    nppDF$eCO2_sd <- nppDF$eCO2_conf / 1.96    
    deltaDF$aCO2_sd <- deltaDF$aCO2_conf / 1.96
    deltaDF$eCO2_sd <- deltaDF$eCO2_conf / 1.96
