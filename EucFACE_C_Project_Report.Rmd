...
``` {r, echo=F, warning=F, message=F, fig.width = 7, fig.align="center"}
    ...
    p <- ggplot(microbial_c_pool, aes(date, Cmic_g_m2, color=factor(ring))) +
        geom_line() + geom_point(size=5) +
...
Below is the fixed coeffcient estimate of understorey respiration flux. 
...
Below is the Q10 estimate of understorey respiration flux.
``` {r, echo=FALSE, warning=FALSE, message=FALSE, fig.width = 7, fig.align="center"}
    understorey_respiration_flux_2 <- make_understorey_respiration_flux(c_pool=understorey_aboveground_c_pool,
                                                                  c_frac=c_fraction,
                                                                  assumption="fixed")

   ### make the plot
    p <- ggplot(understorey_respiration_flux_2,
                aes(Date, respiration, color=factor(Ring))) +   
            geom_point(size = 5) + geom_line() + 
            xlab("Date") + ylab("Understorey respiration flux (mg m-2 d-1)") + 
            scale_color_manual(values=c("#FF7F50", "#00FFFF", "#6495ED",
...
Here, understorey leaf respiration is scaled by a Q10 function with temperature based on data derived from Whole Tree Chamber exmperiment 3. 

The Q10 equation writes as:

$$R_{root} = 1.73 C_{root} 2.26^{(T_{air} - 15) \over 10}$$
...
``` {r, echo = FALSE, warning=FALSE, message=FALSE} 
    ...
    overall_tables$pool$value <- round(overall_tables$pool$value, 2)
    kable(overall_tables$pool)

```

#### Method In and out
``` {r, echo = FALSE, warning=FALSE, message=FALSE} 
    overall_tables$inout$value <- round(overall_tables$inout$value, 2)
    kable(overall_tables$inout)
```

#### Method NPP
``` {r, echo = FALSE, warning=FALSE, message=FALSE} 
    overall_tables$npp$value <- round(overall_tables$npp$value, 2)
    kable(overall_tables$npp)
```
...
``` {r, echo = FALSE, warning=FALSE, message=FALSE} 
    ...
    tables_by_ring$pool[,2:9] <- round(tables_by_ring$pool[,2:9], 2)
    kable(tables_by_ring$pool)

```

#### Method In and out
``` {r, echo = FALSE, warning=FALSE, message=FALSE} 
    tables_by_ring$inout[,2:9] <- round(tables_by_ring$inout[,2:9], 2)
    kable(tables_by_ring$inout)
```

#### Method NPP
``` {r, echo = FALSE, warning=FALSE, message=FALSE} 
    tables_by_ring$npp[,2:9] <- round(tables_by_ring$npp[,2:9], 2)
    kable(tables_by_ring$npp)
```
...
``` {r, echo = FALSE, warning=FALSE, message=FALSE} 
    ...
    kable(tables_by_year$pool)

```

#### Method In and out
``` {r, echo = FALSE, warning=FALSE, message=FALSE} 
    kable(tables_by_year$inout)
```

#### Method NPP
``` {r, echo = FALSE, warning=FALSE, message=FALSE} 
    kable(tables_by_year$npp)
```
<br/>
<br/>
...
# 7. To-do list
    ...
    - Standing dead pool
    - Understorey respiration scaled with temperature
    - summary table with more comments, especialy with information on missing data
    - Check CUE for overstorey and understorey and try to make sure they are in the expected range
    - changes in pools at annual timestep
    - CH4 calculations
    - Litter pool based on % live information
    ...
    - Calculate NPE based on all methods
    - Temporal trend?
    - Met data
    - Create error bars
