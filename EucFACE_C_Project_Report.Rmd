...
``` {r, echo=FALSE, warning=FALSE, message=FALSE, fig.width = 7, fig.align="center"} 
   ...
   ### make the plot
    ...
    ### Process data by treatment
    ### SLA of different rings are not taken from the same measurement day
    ### Need to group them by month
    sla <- sla_variable
...
``` {r, echo=FALSE, warning=FALSE, message=FALSE, fig.width = 7, fig.align="center"} 
   ...
   ### make the plot
    p <- ggplot(understorey_sla_variable,
                aes(Ring, Understorey_sla_variable)) +   
            geom_bar(stat = "identity", aes(fill=Ring),
                     position="dodge") +
            xlab("Date") + ylab("SLA") 
...
#### 2.3.6 Sapwood C and N fraction
``` {r, echo=FALSE, warning=FALSE, message=FALSE, fig.width = 7, fig.align="center"} 

   sapwood_cn_fraction <- make_sapwood_c_n_fraction()

   ### make the table
   kable(sapwood_cn_fraction, digits=4)
...
### Leaf C Pool - variable SLA
``` {r, echo=FALSE, warning=FALSE, message=FALSE, fig.width = 7, fig.align="center"} 
    leaf_c_pool_mean <- make_leaf_pool(lai_variable, sla_variable, c_fraction, sla_option="mean")
    leaf_c_pool <- make_leaf_pool(lai_variable, sla_variable, c_fraction, sla_option="variable")
...
The leaf C pool based on variable SLA is provided above, below is the leaf C pool based on fixed SLA. 
``` {r, echo=FALSE, warning=FALSE, message=FALSE, fig.width = 7, fig.align="center"} 
   ### make the plot
    p <- ggplot(leaf_c_pool_mean,
                aes(Date, leaf_pool, color=factor(Ring))) +   
            geom_point(size = 2.5) + geom_line() + 
            xlab("Date") + ylab("Canopy C Pool (g m-2)") + 
            scale_color_manual(values=ring_colours, name="Ring")
...
Both methods compare similarly. Method 1:
...
``` {r, echo=F, warning=F, message=F, fig.width = 7, fig.align="center"}
    ...
    micro <- summaryBy(microbial_pool~Ring, data=microbial_c_pool, FUN=mean, keep.names=T, na.rm=T)
...
The mycorrhizal C pool is based on results of incubation. We assume it's top 30 cm soil, and 70% of soil is sand.
``` {r, echo=F, warning=F, message=F, fig.width = 7, fig.align="center"}
    mycorrhizal_c_pool <- make_mycorrhizal_c_pool(soil_bulk_density_variable)

    p <- ggplot(mycorrhizal_c_pool, aes(Date, mycorrhizal_c_pool, color=factor(Ring))) +
        geom_line() + geom_point(size=5) +
        xlab("Date") + ylab("Mycorrhizae C Pool (g m-2)") + 
            scale_color_manual(values=ring_colours, name="Ring")
    if (plot_option) {
            plot(p)
    }
...
``` {r, echo=F, warning=F, message=F, fig.width = 7, fig.align="center"}
    ...
    p <- ggplot(standing_dead_c_pool, aes(Date, wood_pool, color=factor(Ring))) +
        ...
        xlab("Date") + ylab("Standing Dead C Pool (g m-2)") + 
            scale_color_manual(values=c("#FF7F50", "#00FFFF","#8B0000", "#0000FF"), name="Ring")
    if (plot_option) {
            plot(p)
    }
...
Here, fineroot and coarseroot respiration are scaled by a Q10 function with soil temperature based on data derived from Whole Tree Chamber exmperiment 3. 

The coarseroot respiration flux should be lower than what we calcualted here. Because here we assume the entire coarseroot pool is respiring. So close the C budget gap, it may be reasonable to estimate a fraction of coarseroot that is respiring. 


### Understorey respiration flux

Here, since we don't have direct measurement of understorey respiration flux, we have to use literature value of dark respiration of Microlaena. we can calculate understorey respiration flux based on three methods, each with certain uncertainties:

    - fixed Rd coefficient (rd)
    - Scaling with temperature (q10)
    - Fixed CUE approach (cue)
...
``` {r, echo=FALSE, warning=FALSE, message=FALSE, fig.width = 7, fig.align="center"}
    ...
    herbivory_respiration_flux <- make_herbivory_respiration_flux(leaf_consumed=herbivory_leaf_consumption_flux,
                                                              frass_prod=frass_production_flux, method = "diff")
...
There are two possibilities: 1. use actual Q10 function at EucFACE, 2. use Q10 function from WTC.
``` {r, echo=FALSE, warning=FALSE, message=FALSE, fig.width = 7, fig.align="center", results="hide"}

    wood_respiration_flux <- make_wood_respiration_flux()

    ### make the plot
    p <- ggplot(wood_respiration_flux,
                aes(Date, wood_respiration, color=factor(Ring))) +   
            geom_point(size = 1) + geom_line() + 
            xlab("Date") + ylab("Wood respiration flux (mg m-2 d-1)") + 
            scale_color_manual(values=ring_colours, name="Ring")
...
The second way using WTC has some issues.The basal respiration rate is comparably OK, but the stem biomass is big, hence the calculated annual stem respiration flux is large (~1000 g C m-2 yr-1). Hence, the problem is either basal respiration rate is actually a lot lower, or the sapwood biomass is lot lower than what we have here. The sapwood biomass data is based on sapwood depth information. 
...
#
    #### make the plot
    #p <- ggplot(wood_respiration_flux2,
    #            aes(Date, wood_respiration, color=factor(Ring))) +   
    #        geom_point(size = 1) + geom_line() + 
    #        xlab("Date") + ylab("Wood respiration flux (mg m-2 d-1)") + 
    #        scale_color_manual(values=ring_colours, name="Ring")
...
``` {r, echo=FALSE, warning=FALSE, message=FALSE, fig.width = 7, fig.align="center"} 
    ...
    ### make the plot
    p <- ggplot(methane_c_flux,
                aes(Date, methane_flux, color=factor(Ring))) +   
            geom_point(size = 5) + geom_line() + 
            xlab("Date") + ylab("CH4 Flux (mg m-2 d-1)") + 
...
##### This is gap-filled data based on DAMM

``` {r, echo=FALSE, warning=FALSE, message=FALSE, fig.width = 7, fig.align="center", results="hide", include=FALSE} 
    methane_c_flux2 <- make_methane_flux2()
    
    ### make the plot
    p <- ggplot(methane_c_flux2,
                aes(Date, methane_flux, color=factor(Ring))) +   
            geom_point(size = 5) + geom_line() + 
            xlab("Date") + ylab("CH4 Flux (mg m-2 d-1)") + 
            scale_color_manual(values=c("#FF7F50", "#00FFFF", "#6495ED",
    ...
    ### Method comparisons
    m1 <- with(methane_c_flux, sum(methane_flux, na.rm=T) / sum(ndays) * 365 / 1000)
    m2 <- with(methane_c_flux2, sum(methane_flux) / sum(ndays) * 365 / 1000)
    
    ch4.tab <- rbind(m1, m2)
    ch4.tab <- as.data.frame(ch4.tab)
    colnames(ch4.tab) <- "Methane_flux_g_m-2_yr-1"
    ch4.tab$method <- c("raw", "gap-filled")
    kable(ch4.tab, digits=2)
```
...
``` {r, echo=FALSE, warning=FALSE, message=FALSE, fig.width = 7, fig.align="center"} 
   ...
   ### make the plot
    p <- ggplot(leaflitter_pool,
                aes(Date, leaflitter_pool, color=factor(Ring))) +   
            geom_point(size = 5) + geom_line() + 
            xlab("Date") + ylab("Leaf litter pool (g m-2)") + 
            scale_color_manual(values=ring_colours, name="Ring")
...
``` {r, echo=FALSE, warning=FALSE, message=FALSE, fig.width = 7, fig.align="center"} 
   ...
   ### make the plot
    p <- ggplot(insect_pool,
                aes(Date, insect_pool, color=factor(Ring))) +   
            geom_point(size = 5) + geom_line() + 
            xlab("Date") + ylab("Insect pool (g m-2)") + 
            scale_color_manual(values=ring_colours, name="Ring")
...
### 6.4 Biomass increments
This is to plot eCO2 effect on biomass increment over the entire data period. There are multiple sources of uncertainties:

    1. Inconsistent start and end years for different pools. Most of the negative biomass increment might be due to end year is a insect disturbance year;
    2. Data processing assumption (e.g. fixed CUE approach for understorey respiration);
    3. Intra-annual variation in annually-fluctuating pools (e.g. microbial pool).
    
Nevertheless, the biomass increment results are summarized below:
...
# 7. Closing the C gaps

Here, I am going to make some bar plots to compare different ways of calculating GPP, Rsoil and NEP, and check if there is any missing carbon.
...
# 8. Understanding disturbances

### 8.1. Leaf NPP and lerp production
``` {r, echo = FALSE, warning=FALSE, message=FALSE, results="hide"} 
...
# 9. To-do list
    ...
    - VOC data
    - Update Rsoil
    - Update mycorrhizal biomass
    - Estimate mycorrhizal production (possibly flower production)
    - Root mortality and turnover 
    - MAESPA output (GPP, Ra for overstorey and understorey)
    - C balance figure to show where the extra C ended up.
    - C budget figure to close the balance
    - C budget figure for NEP
    - power analysis
    - Standardize response by something (e.g. wood biomass) to show a clearer response ratio
    - Update summary tables here and in the manuscript

<br/> 

below are non-major to-do list:

    - The proportion of mycorrhizae in the microbial pool needs to be cross-checked with literature value
    - Flux of mycorrhizae - colonization rate (Check with Yolima)
    - Temporal variation of mycorrhizae (2013 - 15, 3 data points, then 2015-16). 
    - Some fungi get C from dead plant so no GPP involved there. Microbial biomass has a subset that is fungal biomass, then fungal biomass can be splitted into mycorrhizae and saprotrophs. 
    - There is respiration associated with root hyphae and of fungi itself. Juan's respiration data has root-associated mycorrhizae in it, whereas Laura's data is non-root associated mycorrhizae.     
    - Yolima and Luara to check lipid concentration inside and outside mesh bag to constrain concentration differences
    - frass production and relationship with leaf N
    - R soil - inter-collar variability
    - Update Rsoil from Alexis result
    - Check soil C deep layer pools
    - Understorey gpp needs to close the balance with production and respiration
    ...
    - Understorey biomass phenocamera estimates - pretreatment (Matthias)
    - Australian forest CUE estimates
    - Leaf + root tissue specific respiration rate to compare leaf + root respiration flux
    - Leaf + root N content
    - Root respiration + stem respiration (Nam Jin + Elise)
    - Coarse root biomass split into different size categories and calculate respiration rates accordingly
    - Insect biomass (Markus has data for canopy, Sarah has relative abundance, data may need to be processed)
    - Root respiration currently is a lump sum of under and overstorey, is it possible to differetiate the two?
    - Mycorryzae biomass data accompanied by root litter decompsoition rate and isotopic measurement
    - Understorey stem turnover - may need a literature value
    - Dead vs. live for shrubs (live is shrub, dead is CWD)
    - Johanna and Yolima have quantification of biomass of root down to clay layer
    - Mark said that Juan may have Dec 2013 root data down to 50 cm
    - Compare turnover of root to FRED database
    - Make a shiny app of EucFACE C budget
<br/>

Attempted list but with problems:

    - Has leaf C concentration changed with treatment? Only found this file with C concentration information (EucFACE_P0020_RA_CARBO_RAW_201302-201309_v1);
    - Lerp production can be cross checked with leaf NPP to understand its effect. Lerp production data up to 2014, not a major psyllid attack year!
    - Herbivory respiration rate can be done using a 2nd method (frass * use efficiency)
    - CH4 calculation gap-filled. Did not make a huge difference.
    - Nam Jin has estimate of root respiration, and our values are in the range.
    - 2nd way of estimating coarse root biomass - equivalent to 1st way anyway. 
    - Cat checked microbial extraction efficiency constant
    - Litter decomposition rate computed 
