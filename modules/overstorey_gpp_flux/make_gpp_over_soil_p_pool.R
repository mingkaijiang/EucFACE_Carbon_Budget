make_gpp_over_soil_p_pool <- function(pDF=soil_p_pool) {

    ### Read soil P concentration data, calculate all-year average for each ring
    pDF$Date <- as.Date(pDF$Date)
    paDF <- summaryBy(soil_p_g_m2~Ring, data=pDF, FUN=mean, keep.names=T, na.rm=T)
    paDF.sd <- summaryBy(soil_p_g_m2~Ring, data=pDF, FUN=sd, keep.names=T, na.rm=T)
    paDF$sd <- paDF.sd$soil_p_g_m2
    ...
    for (j in 1:6) {
        gppDF$soil_P[gppDF$Ring== j] <- paDF$soil_p_g_m2[paDF$Ring==j]
    }
    ...
    p1 <- ggplot(plotDF1, aes(x=soil_P, y=GPP)) + 
        ...
        annotate("text", x=38, y=2400, label= "2013", size=10) +
        ylim(750, 2500) + xlim(20, 40)
    ...
    p2 <- ggplot(plotDF2, aes(x=soil_P, y=GPP)) + 
        ...
        theme(panel.grid.minor=element_blank(),
              ...
              panel.grid.major=element_line(color="grey"),
              legend.position="none")+
        annotate("text", x=38, y=2400, label= "2014", size=10) +
        ylim(750, 2500) + xlim(20, 40)
    ...
    p3 <- ggplot(plotDF3, aes(x=soil_P, y=GPP)) + 
        ylab(expression(paste("GPP (g C ", m^-2, " ", yr^-1, ")"))) +
        xlab(expression(paste("Soil P pool (g P ", m^-2, ")")))+
        ...
        annotate("text", x=38, y=2400, label= "2015", size=10) +
        ylim(750, 2500) + xlim(20, 40)
    ...
    p4 <- ggplot(plotDF4, aes(x=soil_P, y=GPP)) + 
        ...
        theme(panel.grid.minor=element_blank(),
              ...
              panel.grid.major=element_line(color="grey"),
              legend.position="bottom")+
        annotate("text", x=38, y=2400, label= "2016", size=10) +
        ylim(750, 2500) + xlim(20, 40)
    
    pdf("R_other/GPP_vs_soil_P_pool.pdf", width=12, height=8)
    require(cowplot)
