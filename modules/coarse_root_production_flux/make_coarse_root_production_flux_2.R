make_coarse_root_production_flux_2 <- function(inDF) {
    ...
    for (i in 1:length(delta$Date)) {
        ...
        delta$prev_biom[i] <- inDF$coarse_root_pool[inDF$Ring == delta$Ring[i] &
    ...
    delta$diff_g_yr <- (delta$coarse_root_pool - delta$prev_biom) / delta$length * 365
    ...
    delta$coarse_root_production_flux <- delta$influx / 365 * 1000
    
    
    ### prepare output
    outDF <- delta[,c("Start_date", "Date", "Date", "Ring", "coarse_root_production_flux", "length")]
